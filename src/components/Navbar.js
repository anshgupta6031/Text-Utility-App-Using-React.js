...
export default function Navbar(props) {
    ...
    <nav className={`navbar navbar-expand-lg navbar-${props.mode} bg-${props.mode}`}>
        <div className="container-fluid">
            <Link className="navbar-brand" to="/">{props.title}</Link>                  {/* the title sent as a prop is used.....   Instead of anchor tag Link tag is used and in place of href, to is used....... */}
            ...
            <div className="collapse navbar-collapse" id="navbarSupportedContent">
                <ul className="navbar-nav me-auto mb-2 mb-lg-0">
                    <li className="nav-item">
                        <Link className="nav-link" aria-current="page" to="/">{props.home}</Link>                {/* used the default prop. */}
                    </li>
                    <li className="nav-item">
